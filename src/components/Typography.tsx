...
export type TypographyProps = VariantProps<typeof typographyStyles> &
  React.HTMLAttributes<Element> & { children: React.ReactNode; className?: string; htmlTag?: ValidHtmlTag };

export function Typography({ children, htmlTag, intent, className: cn, ...props }: TypographyProps) {
  const idealTag = determineIdealTag(intent);
  const tag = htmlTag && validTags.includes(htmlTag) ? htmlTag : idealTag;
  return React.createElement(
    tag,
    { className: `${typographyStyles({ intent })} ${cn ? cn : ""}`, ...props },
    children
  );
}
